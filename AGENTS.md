...
2. **Device / responsive states.** Prefixes: size `sm:` (≥600), `md:` (≥800), `lg:` (≥1024); orientation `portrait:` (there is no `landscape:` — landscape is the unprefixed base); bit-depth `1bit:` / `2bit:` / `4bit:`; and `dark:`. Mapping: **base = the original/OG TRMNL (smaller screen)**, **`lg:` = TRMNL X (the larger 4-bit screen)**, and `portrait:` = a device mounted portrait. Combine in the order `dark:size:orientation:bit-depth:utility` (e.g. `md:portrait:4bit:gap--large`).
